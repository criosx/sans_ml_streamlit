...
user_sans_model_dir = Path(st.session_state['user_sans_model_dir']).expanduser().resolve()
user_sans_file_dir = Path(st.session_state['user_sans_file_dir']).expanduser().resolve()
...
def _generate_key(path: Path) -> str:
    """
    Generates a unique key for a file path based on the file content
...
def add_par(parname, model_name, fitobj):
    ...
    shutil.copyfile(user_sans_fit_dir / model_name, user_sans_model_dir / model_name)
...
col1_2.text('Add / Remove fit parameters (for single-model fit problems)')
